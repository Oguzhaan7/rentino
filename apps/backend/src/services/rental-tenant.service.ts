    ...
    try {
      ...
      const transformedTenants = tenants.map((tenant) => {
        ...
        return {
          ...
          status: tenantStatus,
          paymentStatus,
          lastPayment: latestContract
            ? new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000)
            : null, // Mock last payment
          ...
          contract: latestContract
            ? {
                ...
                deposit: latestContract.deposit,
              }
            : null,
    ...
    try {
      ...
      const tenant = await prisma.user.create({
        data: {
          ...
          avatar,
          role: "TENANT",
          password: "temp_password", // In real app, generate temporary password or send invitation
          isActive: true,
        },
      });
