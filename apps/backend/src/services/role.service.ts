...
export const roleService = {
    ...
    try {
      ...
      const roleConfigs = [
        ...
        },
        {
          ...
          name: "Kiracı",
          description:
            "Kiracı kullanıcılar, kendi bilgilerini görüntüleyebilir",
          color: "orange",
          icon: "User",
          permissions: [
            "profile_view",
      ...
      const rolesWithCounts = roleConfigs.map((role) => {
        const userCount =
          userCounts.find((count) => count.role === role.id)?._count.id || 0;
        return {
          ...role,
          userCount,
          isActive: true,
      ...
      return reply
        .status(500)
        .send({ error: "Roller listelenirken hata oluştu" });
    }
  },
  ...
  async getRolePermissions(
    ...
    try {
      ...
      for (const [groupName, groupPermissions] of Object.entries(
        permissionGroups
      )) {
        const rolePerms =
          groupPermissions[role as keyof typeof groupPermissions] || [];
      ...
      return reply
        .status(500)
        .send({ error: "Rol izinleri alınırken hata oluştu" });
    }
  },
  ...
  async getRoleStats(
    ...
    try {
      ...
      return reply
        .status(500)
        .send({ error: "Rol istatistikleri alınırken hata oluştu" });
    }
  },
  ...
  async updateRolePermissions(
    ...
    try {
      ...
      return {
        success: false,
        message:
          "Rol izinleri şu anda özelleştirilemez. Bu özellik gelecek sürümlerde eklenecektir.",
      };
    } catch (error) {
      request.log.error(`Update role permissions error: ${error}`);
      return reply
        .status(500)
        .send({ error: "Rol izinleri güncellenirken hata oluştu" });
    }
  },
};
