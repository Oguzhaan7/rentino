<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <Button
        v-if="userStore.canManageContracts"
        ...
        <CardContent>
          ...
          <div class="mt-4 flex gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              @click="viewPayments(contract.id)"
            >
            ...
            <Button
              v-if="
                contract.status === 'ACTIVE' && userStore.canManageContracts
              "
              variant="destructive"
              size="sm"
...
import { Plus, Eye, Edit, CreditCard, X, Contract } from "lucide-vue-next";
...
const filteredContracts = computed(() => {
  ...
  if (typeFilter.value !== "all") {
    filtered = filtered.filter(
      (contract) => contract.type === typeFilter.value
    );
  }
